...
redEng = Engine(robotfile="PyrobotRobot60000.py",
                simfile="PyrobotSimulator",
                config=pyroConfig,
                worldfile="redFlagWorld.py")
blueEng = Engine(robotfile="PyrobotRobot60001.py")

def eval_fitness(population):
    """
    Evaluate the fitness of the given population.
    """
    print "Evaluating the RED robot..."
...
def main(argv = None):
    ...
    if chromo_file:
        ...
        pop.epoch(23, report=True, save_best=True, checkpoint_interval=4, \
                  checkpoint_generation=None, name="red")
        # changed from 4, None, 2
    
        # Plots the evolution of the best/average fitness (requires Biggles)
        visualize.plot_stats(pop.stats, name="red")
        # Visualizes speciation
        visualize.plot_species(pop.species_log, name="red")
        winner = pop.best
        # Visualize the winner network (requires PyDot)
        visualize.draw_net(winner, "red") # best chromosome
